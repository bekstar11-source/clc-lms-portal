...
import { 
  ...
  MessageCircle, AlertTriangle, Medal, ChevronRight, TrendingUp
} from 'lucide-react';
...
const StudentDashboard = () => {
  ...
  // --- CACHING & FETCHING (Oldingidek qoladi) ---
  const loadFromCache = () => {
  ...
  const handleTabChange = (tab) => { triggerHaptic('tap'); setActiveTab(tab); window.scrollTo({ top: 0, behavior: 'smooth' }); };
  ...
  return (
    <div className="min-h-[100dvh] bg-slate-50 font-sans pb-28 md:pb-12 touch-manipulation">
      ...
      <div className="max-w-7xl mx-auto space-y-6 p-4">
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {/* 🔥 1. COMPACT PROFILE CARD (IXCHAM & REARRANGED) */}
            ...
            {/* 🔥 2. ACTION ITEMS (ALERT) - ACCORDION STYLE */}
            {actionItems.length > 0 && (
                <div className="bg-red-500 rounded-2xl shadow-lg shadow-red-200 overflow-hidden transition-all duration-300 border border-red-400">
                    <button 
                        onClick={() => { triggerHaptic(); setIsAlertsExpanded(!isAlertsExpanded); }} 
                        className="w-full flex items-center justify-between p-4 text-white active:bg-red-600 transition-colors"
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <div className="text-left">
                                <h3 className="text-sm font-black uppercase tracking-wide leading-none">Diqqat Talab!</h3>
                                <p className="text-[10px] font-medium opacity-90 mt-1">{actionItems.length} ta muammo mavjud</p>
                            </div>
                        </div>
                                ...
                                <div key={idx} onClick={() => setActiveTab(item.type === 'retake' ? 'grades' : 'schedule')} className="flex items-center justify-between p-3 bg-red-50 border border-red-100 rounded-xl active:scale-98 transition-transform cursor-pointer">
                                    <div className="flex-1 min-w-0 pr-3">
                                        <div className="flex items-center gap-2 mb-1">
                                            <span className={`text-[8px] font-black uppercase px-1.5 py-0.5 rounded ${item.type === 'retake' ? 'bg-red-200 text-red-700' : 'bg-orange-200 text-orange-700'}`}>
                                                {item.type === 'retake' ? "Retake" : "Missing"}
                                            </span>
            ...
            {/* 5. CHART AREA (ENG PASTDA) */}
            <div className="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm mb-20">
                  ...
                  {isExpanded && (
                    <div className="p-4 space-y-4 border-t border-slate-100 bg-slate-50/50">
                      {monthLessons.map((lesson) => {
                        ...
                        // Only mark as missing if not delayed
        ...
        {activeTab === 'grades' && (
          <div className="space-y-4 animate-in fade-in pb-20">
            ...
            <div className="space-y-3">
              {[...grades].map((g, i) => {
                const isRetakeNeeded = g.score < 60; // 60 dan past bo'lsa retake
                let daysLeft = null;
                // RETAKE TIMER
                if (g.retakeDeadline) {
                ...
                return (
                  <div key={i} className={`p-5 rounded-[2rem] border bg-white shadow-sm flex flex-col gap-3 ${isRetakeNeeded ? 'border-amber-400 ring-2 ring-amber-100' : 'border-slate-100'}`}>
                    ...
                    {/* SHOW COUNTDOWN IF RETAKE NEEDED */}
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 w-full bg-white/95 backdrop-blur-xl border-t border-slate-200 flex justify-around py-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] z-50 shadow-[0_-5px_20px_-5px_rgba(0,0,0,0.05)]">
        ...
        <button onClick={() => {triggerHaptic(); navigate('/chat');}} className="flex flex-col items-center gap-1 p-2 rounded-2xl transition-all active:scale-95 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 relative">
